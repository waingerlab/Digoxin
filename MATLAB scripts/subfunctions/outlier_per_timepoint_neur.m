function[removed_outliers_TR]=outlier_per_timepoint_neur(TR_2, n, removed_outliers_TR, TR,num_timepoints)
...
for bb=1:height(TR_2)
    if (TR_2(bb,n+2*numel(num_timepoints)) > upq_TR_Neur+(1.5*iqr_TR_Neur)) 
        removed_outliers_TR(cc,:)= TR(bb,:);
        removed_outliers_TR(cc, end) = {[strcat(timepoint, ' > ' ,num2str(upq_TR_Neur+(1.5*iqr_TR_Neur)))]};
        cc=cc+1;
    elseif TR_2(bb, n+2*numel(num_timepoints)) < lowq_TR_Neur-(1.5*iqr_TR_Neur)
        removed_outliers_TR(cc,:)= TR(bb,:);
