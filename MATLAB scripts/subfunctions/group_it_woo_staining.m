function[all_treats,treats_combined]=group_it_woo_staining(data,cond_assigns,condition)
...
for g=1:numel(every_cond)
    j=0;
    if g==1
        all_treats=[all_treats;every_cond(1,g)];
...
for i=1:numel(all_treats)
    ...
    for u=1:numel(every_cond)
        if contains(string(every_cond(u,1)), string(all_treats(i,1)))
